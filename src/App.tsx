...
function App() {
  ...
  useEffect(() => {
    ...
    presenceChannel.bind("pusher:member_added", (member: OnlineUser) => {
      setOnlineUsers((prev) => {
        const exists = prev.some((u) => Number(u.id) === Number(member.id));
  ...
  useEffect(() => {
    ...
    channel.bind("MessageSentEvent", (data: LiveMessage) => {
      ...
      setConversationMessages((prev) => {
        ...
        // Type merging
        const newMessage = transformLiveToChat(data, loggedUser, targetUser);
  ...
  }, [loggedUser?.auth_token, targetUser]);

  return (
    <>
      <BrowserRouter>
        <Suspense fallback={<Helix size="45" speed="2.5" color="black" />}>
